...
from attacks.tools import is_float_or_torch_tensor
from attacks.tools import batch_multiply
from attacks.tools import batch_clamp
from attacks.tools import replicate_input
from attacks.tools import batch_l1_proj
...
def clamp_(X, lower_limit, upper_limit):
    return torch.max(torch.min(X, upper_limit), lower_limit)


def perturb_iterative(xvar, yvar, predict, sa_b, nb_iter, eps, eps_iter, loss_fn,
    ...
    a = sa_b
    delta.data = torch.mul(delta.data, a.cuda())
    delta = clamp_(delta, torch.tensor(-eps).cuda(), torch.tensor(eps).cuda())
    xadv = clamp_(xvar + delta, lower_limit.cuda(), upper_limit.cuda())
    # import pdb; pdb.set_trace()
    xadv = xadv + delta
    return xadv
...
class PGDAttack(Attack, LabelMixin):
    ...
    def perturb(self, x, y=None,sa_b=None):
        ...
        # import pdb; pdb.set_trace()
        # delta.data = torch.mul(delta.data, torch.zeros(delta.data.shape).cuda())
        rval = perturb_iterative(
            x, y, self.predict, sa_b, nb_iter=self.nb_iter,
...
class LinfBasicIterativeAttack(PGDAttack):
    ...
    def __init__(self, predict, loss_fn=None, eps=0.1, nb_iter=10, eps_iter=0.05,
                    clip_min=0., clip_max=1., rand_init=True, targeted=False):
        ord = np.inf
        # rand_init = False
...
class MomentumIterativeAttack(Attack, LabelMixin):
    ...
    def __init__(
            ...
            decay_factor=1., clip_min=0., clip_max=1., targeted=False,
            ord=np.inf):
